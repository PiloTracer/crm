...
export function isSafeStringRe(inputString: string, key: string): boolean {
    const sanitized = sanitizeChars(inputString, key);
    return sanitized.length === inputString.length;
}


export function sanitizeChars(inputString: string, key: string): string {
    if (key === "email") {
    ...
    for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
            ...
            if (typeof value === 'string') {
                result = isSafeStringRe(value, key);
